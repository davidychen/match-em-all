...
function init() {
  ...
  if (Meteor.isServer) {
    ...
    for (var i = 0;i<total/2;i++) {
      var now = parseInt(Math.random() * (300 - 1) + 1);
      matrix.push(now);
      matrix.push(now);
    }
...
Meteor.methods({
  "pokemon.flip"(index) {
    ...
    for (let i = 0;i < board.length;i++) {
      if (board[i].id == board[index].id && board[i].user == this.userId && i != index) {//flip it back
    ...
    if (nowFlip.length >= 3) { //flip it back after 3 times
      ...
      Meteor.setTimeout(function flipBack(){ //flip it back after 5 seconds
        board = Pokemon.findOne({}).board;
